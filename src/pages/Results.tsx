...
const Results = () => {
  ...
  const fetchHealthChecks = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('health_checks').select().order('timestamp', {
        ascending: false
      }).limit(20);
  ...
  const handleDeleteRecord = async (id: string) => {
    try {
      const {
        error
      } = await supabase.from('health_checks').delete().eq('id', id);
      if (error) throw error;
  ...
  useEffect(() => {
    fetchHealthChecks();
    const channel = supabase.channel('health_checks_changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'health_checks'
    }, () => {
  ...
  if (healthChecks.length === 0) {
    return <div className="min-h-screen bg-secondary dark:bg-gray-900 p-8 text-center">
  ...
  return <div className="min-h-screen bg-secondary dark:bg-gray-900 p-8">
      <div className={`${theme === 'light' ? 'bg-white' : 'bg-darkBlue-DEFAULT dark:bg-gray-900'} text-white py-4 px-8 shadow-md dark:shadow-black/30 mb-8`}>
        <div className="max-w-6xl mx-auto flex items-center">
          <div className="flex-1">
            <img src="/lovable-uploads/c8b4cabf-f0ee-4d05-883b-4070fbf16a5e.png" alt="SmartShift Logo" className="h-6" />
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        <div className="text-center mb-12">
          ...
          <div className="mt-4 flex justify-center">
            <Button onClick={() => navigate('/')} className="text-slate-900">Submit Another Response</Button>
          </div>
        </div>

        {Object.entries(groupedHealthChecks).map(([date, checks]) => <HealthCheckGroup key={date} date={date} checks={checks} onDelete={handleDeleteRecord} />)}
      </div>
    </div>;
};
